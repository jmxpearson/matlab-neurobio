...
stim_rates = exp(log(effect * baseline) * (1 + trial_noise_std * randn(Ntrials, 1)));

% generate random spike counts
baseline_counts = poissrnd(baseline_rates * Tbaseline);
stim_counts = poissrnd(stim_rates * Tstim);
...
% test
[~, pval] = ttest(obs_baseline_rates, obs_stim_rates); 